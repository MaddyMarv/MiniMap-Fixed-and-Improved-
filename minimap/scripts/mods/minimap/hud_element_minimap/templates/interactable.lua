...
template.update_function = function(widget, marker, x, y, vertical_distance, range, is_out_of_range)
    ...
    local interaction_icon = nil
    if type(marker.data) == "table" or type(marker.data) == "userdata" then
        local success, result = pcall(function() return marker.data:interaction_icon() end)
        if success then
            interaction_icon = result
        end
    end
    ...
    local vertical_str = ""
    if mod and mod:get("distance_marker_vertical_symbols") and is_tagged then
